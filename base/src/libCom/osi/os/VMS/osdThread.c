...
//#if 0			// the C run time library has a problem with
			// the returned nanoseconds (fixed in the next
			// version). Therefore, we avoid all the signal
			// wakeups by using lib$wait.

    delayTime.tv_sec = (time_t)seconds;
    nanoseconds = (seconds - (double)delayTime.tv_sec) *1e9;
    delayTime.tv_nsec = (long)nanoseconds;

//    while ( nanosleep(&delayTime,&remainingTime) ) {
//	if ( errno != 4 ) break;
//	delayTime = remainingTime;
//	delayTime.tv_sec = remainingTime.tv_sec;
//	delayTime.tv_nsec = remainingTime.tv_nsec * 10000;
//	if ( delayTime.tv_nsec > 999999999 ) delayTime.tv_nsec = 999999999;
//    }
//    nanosleep(&delayTime,&remainingTime);	// original POSIX, buggy
